...
class ConnectivityInfo extends Cubit<ConnectivityInfoState>
  ...
  Future<void> update() async {
    ...
    List<Future> futures = [
      _session.getExternalAddrV4().then(
        (externalAddressV4) => emitUnlessClosed(
          state.copyWith(externalAddressV4: externalAddressV4 ?? ""),
        ),
      ),
      _session.getExternalAddrV6().then(
        (externalAddressV6) => emitUnlessClosed(
    ...
    futures.add(
      _networkInfo.getWifiIP().then((localIPv4) {
        if (localIPv4 != null) {
          final port =
              listenerAddrs
    ...
    futures.add(
      _networkInfo.getWifiIPv6().then((localIPv6) {
        if (localIPv6 != null) {
          final port =
              listenerAddrs
              ...
              "";
          emitUnlessClosed(
            state.copyWith(localAddressV6: "[$localIPv6]:$port"),
          );
        } else {
