...
class _AccessModeDropDownMenuState extends State<AccessModeDropDownMenu>
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: DropdownButton(
          ...
          items: widget.accessModes
              .map((AccessMode element) => DropdownMenuItem(
                  value: element,
                  child: _buildOpenAccessModeItem(context, element)))
              .toList(),
          selectedItemBuilder: (context) =>
              widget.accessModes.map<Widget>((AccessMode element) {
            return _buildAccessModeItem(element);
          }).toList(),
          onChanged: (accessMode) async {
            loggy.app('Access mode: $accessMode');
            setState(() {
              _accessMode = accessMode as AccessMode;
            });
  ...
  Widget _buildAccessModeItem(AccessMode accessMode) => Padding(
