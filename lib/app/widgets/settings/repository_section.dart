...
class RepositorySectionMobile extends s.AbstractSettingsSection
    with AppLogger, RepositoryActionsMixin {
  ...
  Widget build(BuildContext context) => repos.builder(
        (repos) => s.SettingsSection(
          title: Text(S.current.titleRepository),
          tiles: [
            s.SettingsTile(title: RepositorySelector(repos)),
  ...
  s.AbstractSettingsTile _buildCurrentTile(
    ...
    return s.CustomSettingsTile(child: widget);
  }
  ...
  Widget _buildRenameTile(BuildContext context, _) => NavigationTile(
      title: Text(S.current.actionRename),
      leading: Icon(Icons.edit),
  ...
  Widget _buildShareTile(BuildContext context, RepoCubit repo) =>
      NavigationTile(
          title: Text(S.current.actionShare),
          leading: Icon(Icons.share),
          onTap: () async => await shareRepository(context, repository: repo));

  Widget _buildSecurityTile(BuildContext parentContext, _) => NavigationTile(
      title: Text(S.current.titleSecurity),
      leading: Icon(Icons.password),
      onTap: () async => await onRepositorySecurity(parentContext));
  ...
  Widget _buildDeleteTile(BuildContext context, RepoCubit repo) =>
      NavigationTile(
        title: Text(S.current.actionDelete,
            style: const TextStyle(color: Constants.dangerColor)),
        leading: Icon(Icons.delete, color: Constants.dangerColor),
        onTap: () async => await onDeleteRepository(context),
      );
}
