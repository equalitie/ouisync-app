...
public class ReplicatedFileProvider: NSObject, NSFileProviderReplicatedExtension, NSFileProviderServicing {
    static let server = Future<Server> { try await .init(configDir: Constants.configsPath,
                                                         debugLabel: "service") }
    ...
    required public init(domain: NSFileProviderDomain) {
        client = .init { try await Self.server.value.connect() }
        log = Log("ReplicatedFileProvider/\(domain.displayName)")
        log("Init")
        super.init()
    }
    ...
    /** Called before the instance is discarded.
     ...
     deallocated. */
    public func invalidate() { log("Invalidating") }
    deinit { log("Gone") }
    ...
    public func enumerator(for containerItemIdentifier: NSFileProviderItemIdentifier,
        ...
        log.info("Requested enumerator for \(containerItemIdentifier)")
        return Enumerator(client, containerItemIdentifier)
    }
    ...
    public func supportedServiceSources(for itemIdentifier: NSFileProviderItemIdentifier,
                                        completionHandler: @escaping ([any NSFileProviderServiceSource]?,
                                                                      (any Error)?) -> Void) -> Progress {
        completionHandler([], nil)
        return .init()
    }

    // MARK: head / get / put / patch / delete
    ...
    // TODO: consider adding conformance to the following protocols:
    // NSFileProviderThumbnailing post-selective sync: generate, upload & fetch thumbs for preview
    // NSFileProviderIncrementalContentFetching this is for rsync (e.g. transfer optimization)
    // NSFileProviderPartialContentFetching (12.3+) this could possibly allow incremental streaming
    // NSFileProviderExternalVolumeHandling (15+) storing repositories on external volumes
     ...
     */
    public func importDidFinish() async {
        log.info("Called importDidFinish")
    }
    ...
    /** Called by the system if the set of materialized items changes.
     ...
     ---------------
     This call is not expected to take more than a few seconds to complete. */
    public func materializedItemsDidChange() async {
        log.info("Called materializedItemsDidChange")
    }
    ...
    public func pendingItemsDidChange() async {
        log.info("Called pendingItemsDidChange")
    }
}
