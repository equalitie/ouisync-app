...
void main() {
  ...
  testWidgets(
    ...
    (tester) => tester.runAsync(() async {
      ...
      final actualFileContent = await readRepoFileAsString(repoCubit, fileName);
  ...
  testWidgets(
    ...
    (tester) => tester.runAsync(() async {
      ...
      await deps.reposCubit.bottomSheet.waitUntil(
        (state) => state is SaveMediaSheetState,
      );
      await tester.pump();
  ...
  testWidgets(
    ...
    (tester) => tester.runAsync(() async {
      ...
      mediaReceiverController.add([
        SharedMediaFile(path: repoPath, type: SharedMediaType.file),
      ]);
    ...
    testWidgets(
      ...
      (tester) => tester.runAsync(() async {
        ...
        await tester.pumpWidget(
          testApp(createMainPage(), navigatorObservers: [navigationObserver]),
        );
        await tester.pumpAndSettle();

        mediaReceiverController.add([
          SharedMediaFile(path: token.toString(), type: type),
        ]);
        ...
        expect(
          deps.reposCubit.state.repos.values.where(
